...
public class Migrate55to56 implements MigrationTask
{
    ...
    public boolean isActive(final Version currentVersion) {
        if (currentVersion.toMajorMinorString().equals("5.6")) {
    ...
    private boolean migrateConfigs() {
        try {
            final String path5_5 = Joiner.on(File.separator).join((Object)Constants.JOURNEYMAP_DIR, (Object)"config", new Object[] { "5.5" });
            final File legacyConfigDir = new File(FileHandler.MinecraftDirectory, path5_5);
            if (!legacyConfigDir.canRead()) {
                return true;
            }
            this.logger.info("Migrating configs from 5.5 to 5.6");
            ...
            Journeymap.getClient().getCoreProperties().optionsManagerViewed.set("5.5");
            return true;
        }
        catch (Throwable t2) {
